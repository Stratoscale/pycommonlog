...
class Formatter(object):
    ...
    def __init__(self, relativeTime, withThreads, showFullPaths, minimumLevel, microsecondPrecision, noColors,
        ...
        try:
            self.configFile = yaml.safe_load(open(LOG_CONFIG_FILE_PATH, 'r').read())
...
def runRemotely(host, ignoreArgs):
    try:
        configFile = yaml.safe_load(open(LOG_CONFIG_FILE_PATH, 'r').read())
    except:
        configFile = {}
...
class LogPathFinder:
    def __init__(self, confFile):
        try:
            self.confFile = yaml.safe_load(open(confFile, 'r').read())
        except:
            pass
        self.defaultPaths = []
...
def main():
    ...
    parser.add_argument("--since", type=str, metavar='DATE', help='Show entries not older than the specified date '
                        '(e.g., 1h, 5m, two hours ago, 8/aug/1997)')
    parser.add_argument("--until", type=str, metavar='DATE', help='Show entries not newer than the specified date '
                        '(e.g., 0.5h, 4m, one hour ago)', default=None)
